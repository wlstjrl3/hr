<?php
    ...
    $sql = "SELECT 
        ...
        ,C.TRS_DT, B.ORG_NM, B.PERSON_CNT,
        A.PSNL_CD,A.PSNL_NM,A.BAPT_NM,A.PHONE_NUM,LEFT(A.PSNL_NUM,6) AS PSNL_NUM
        ...
        ,IFNULL((
            SELECT SUM(ADJ_PAY) FROM PSNL_ADJUST WHERE ADJ_TYPE='장애인' AND PSNL_CD = A.PSNL_CD
            AND LEFT(ADJ_STT_DT,4) <= LEFT(D.ADVANCE_DT,4)
            AND (LEFT(ADJ_END_DT,4) >= LEFT(D.ADVANCE_DT,4) OR ADJ_END_DT is null)
        ),0) AS ADJUST_PAY3
        ,IFNULL((
