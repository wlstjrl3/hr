...
// 갯수 카운트 쿼리
$rowCntSql = "SELECT COUNT(*) AS ROW_CNT FROM BONDANG_HR.ORG_FINANCIAL F JOIN BONDANG_HR.ORG_INFO O ON F.ORG_CD = O.ORG_CD";

// 기본 쿼리
$sql = "SELECT CONCAT(F.FSC_YEAR, '|', F.ORG_CD, '|', F.ACC_NM) AS UID, F.FSC_YEAR, F.ORG_CD, O.ORG_NM, F.ACC_NM, F.AMOUNT
...
// 정렬 기준 지정
$orderSql = safeOrderBy(@$_REQUEST['ORDER'], ['FSC_YEAR', 'ORG_CD', 'ORG_NM', 'ACC_NM', 'AMOUNT']);
