...
$grdCond = "";
if ($baseYear) {
    $baseYearEsc = mysqli_real_escape_string($conn, $baseYear);
    $trsCond = " AND LEFT(TRS_DT, 4) <= '{$baseYearEsc}' ";
    $pttCond = " WHERE PTT_YEAR <= '{$baseYearEsc}' ";
    $grdCond = " WHERE LEFT(ADVANCE_DT, 4) <= '{$baseYearEsc}' ";
}
...
if (@$_REQUEST['STAT_MODE'] == '1') {
    ...
    if ($baseYear) {
        $whereSql .= " AND NULLIF(GREATEST(IFNULL(LEFT(D.ADVANCE_DT, 4), '0000'), IFNULL(PTT.PTT_YEAR, '0000')), '0000') = ?";
        $params[] = $baseYear;
        $types .= "s";
    }
