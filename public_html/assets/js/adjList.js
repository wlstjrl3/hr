...
function trDataXHR(idx) {
    ...
    fetch(url)
        ...
        .then(json => {
            ...
            if (res != null) {
                ...
                document.querySelector(".modalBody").querySelectorAll("input").forEach((input, key) => {
                    switch (key) {
                        case 0: input.value = res[0].ADJ_CD; break;
                        case 1: input.value = res[0].ADJ_NM; break;
                        case 2: input.value = res[0].ADJ_NUM; break;
                        case 3: input.value = res[0].ADJ_LEVEL; break;
                        case 4: input.value = res[0].ADJ_GET_DT; break;
                        case 5: input.value = res[0].ADJ_DTL; break;
                        case 6: input.value = res[0].ADJ_PAY; break;
                        case 7: input.value = res[0].ADJ_STT_DT; break;
                        case 8: input.value = res[0].ADJ_END_DT; break;
                    }
                });
        ...
        .catch(error => {
            console.error("adjConfigXhr 정보 로드 에러:", error);
        });
}
...
modalDelBtn.addEventListener("click", () => {
    ...
    fetch(url)
        ...
        .catch(error => {
            console.log("adjConfig 정보 제거 에러!!!:", error);
        });
});
//검색 필터링을 위한 코드
