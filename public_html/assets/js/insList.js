...
function trDataXHR(idx) {
    ...
    fetch(url)
        ...
        .then(json => {
            ...
            if (res != null) {
                ...
                document.querySelector(".modalBody").querySelectorAll("input").forEach((input, key) => {
                    switch (key) {
                        case 0: input.value = res[0].INS_CD; break;
                        case 1: input.value = res[0].INS_AMOUNT; break;
                        case 2: input.value = res[0].INS_STT_DT; break;
                        case 3: input.value = res[0].INS_END_DT; break;
                        case 4: input.value = res[0].INS_DTL; break;
                    }
                });
        ...
        .catch(error => {
            console.error("insConfigXhr 정보 로드 에러:", error);
        });
}
...
modalEdtBtn.addEventListener("click", () => {
    ...
    fetch(url)
        ...
        .catch(error => {
            console.log("insConfig 정보 기록 에러!!!:", error);
        });
});
...
modalDelBtn.addEventListener("click", () => {
    ...
    fetch(url)
        ...
        .catch(error => {
            console.log("insConfig 정보 제거 에러!!!:", error);
        });
});
//검색 필터링을 위한 코드
