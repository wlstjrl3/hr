...
function trDataXHR(idx) {
    ...
    fetch(url)
        ...
        .then(json => {
            ...
            if (res != null) {
                /* 급호봉관리 페이지로 이동버튼 */
                document.getElementById("goGrdListBtn").onclick = () => { location.href = DIR_ROOT + "/grdList?PSNL_CD=" + res[0].PSNL_CD + "&PSNL_NM=" + res[0].PSNL_NM + "&POSITION=" + res[0].POSITION + "&ORG_CD=" + res[0].ORG_CD + "&ORG_NM=" + res[0].ORG_NM + "&WORK_TYPE=" + res[0].WORK_TYPE };
                ...
                document.querySelector(".modalBody").querySelectorAll("input").forEach((input, key) => {
                    switch (key) {
                        case 0: input.value = res[0].TRS_CD; break;
                ...
                document.querySelector(".modalBody").querySelectorAll("select").forEach((sel, key) => {
                    switch (key) {
                        case 0: sel.value = res[0].WORK_TYPE; break; //재직구분
                        case 1: sel.value = res[0].POSITION; break; //직책
                        case 2: sel.value = res[0].TRS_TYPE; break; //인사구분
                    }
                });
        ...
        .catch(error => {
            console.error("trsConfigXhr 정보 로드 에러:", error);
        });
}
...
modalEdtBtn.addEventListener("click", () => {
    ...
    fetch(url)
        ...
        .catch(error => {
            console.log("trsConfig 정보 기록 에러!!!:", error);
        });
});
...
modalDelBtn.addEventListener("click", () => {
    ...
    fetch(url)
        ...
        .catch(error => {
            console.log("trsConfig 정보 제거 에러!!!:", error);
        });
});
//검색 필터링을 위한 코드
