...
function trDataXHR(idx) {
    ...
    xhr.open("GET", DIR_ROOT + "/sys/trsConfig.php?key=" + psnlKey.value + "&TRS_CD=" + idx + "&CRUD=R"); xhr.send(); //XHR을 즉시 호출한다. psnlKey는 추후 암호화 하여 재적용 예정
    console.log(DIR_ROOT + "/sys/trsConfig.php?key=" + psnlKey.value + "&TRS_CD=" + idx + "&CRUD=R");
    xhr.onload = () => {
        if (xhr.status === 200) { //XHR 응답이 존재한다면
            var res = JSON.parse(xhr.response)['data']; //응답 받은 JSON데이터를 파싱한다.
            if (res != null) {
                /* 급호봉관리 페이지로 이동버튼 */
                document.getElementById("goGrdListBtn").addEventListener("click", () => { location.href = DIR_ROOT + "/grdList?PSNL_CD=" + res[0].PSNL_CD + "&PSNL_NM=" + res[0].PSNL_NM + "&POSITION=" + res[0].POSITION + "&ORG_CD=" + res[0].ORG_CD + "&ORG_NM=" + res[0].ORG_NM + "&WORK_TYPE=" + res[0].WORK_TYPE });
...
modalEdtBtn.addEventListener("click", () => {
    ...
    console.log(DIR_ROOT + "/sys/trsConfig.php?key=" + psnlKey.value + writeUrl + "&CRUD=C");
    xhr.open("GET", DIR_ROOT + "/sys/trsConfig.php?key=" + psnlKey.value + writeUrl + "&CRUD=C"); xhr.send(); //XHR을 즉시 호출한다. psnlKey는 추후 암호화 하여 재적용 예정
    xhr.onload = () => {
        if (xhr.status === 200) { //XHR 응답이 존재한다면
...
modalDelBtn.addEventListener("click", () => {
    ...
    console.log(DIR_ROOT + "/sys/trsConfig.php?key=" + psnlKey.value + deleteUrl + "&CRUD=D");
    xhr.open("GET", DIR_ROOT + "/sys/trsConfig.php?key=" + psnlKey.value + deleteUrl + "&CRUD=D"); xhr.send(); //XHR을 즉시 호출한다.
    xhr.onload = () => {
        if (xhr.status === 200) { //XHR 응답이 존재한다면
            //var res = JSON.parse(xhr.response)['data']; //응답 받은 JSON데이터를 파싱한다.
